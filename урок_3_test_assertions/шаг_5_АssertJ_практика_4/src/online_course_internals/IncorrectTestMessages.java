...
class IncorrectTestMessages {
    ...
    static String noFailWhenBuggyImplementation(String testedMethodName) {
        return String.format("Ваш тест %s не падает, когда тестирует некорректную реализацию. (или же падает, но не по причине проверки)", testedMethodName);
    }
    ...
    static String unexpectedTestedValue(String testedMethodName, String unexpectedTestedValueName) {
        return String.format("Ваш тест %s должен тестировать %s.", testedMethodName, unexpectedTestedValueName);
    }
}
